        ...
        for(const auto& mapLayer : mapData.layers)
        {
            for (int i = 0; i < mapLayer->width; i++)   
            {
                for (int j = 0; j < mapLayer->height; j++)
                {
                   ...
                   Vector2D mapCoords = MapToWorld(i, j);
                   
                    //Draw the texture

                   Engine::GetInstance().render.get()->DrawTexture(mapData.tilesets.front()->texture, mapCoords.getX(), mapCoords.getY(), &tileRect);
                }
            }
        }
    }
...
Vector2D Map::MapToWorld(int x, int y)
{
    Vector2D pos;

    pos.setX(x * mapData.tileWidth);
    pos.setY(y * mapData.tileHeight);
    return pos;
